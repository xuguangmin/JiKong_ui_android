...
import phlx.jikong.utils.msg.*;
import phlx.jikong.device.net.*;
import phlx.jikong.device.protocol.*;
...
public class Device implements Runnable, ITcpClient, IUdpBroadcast, IRecvBuffer, IProtocol
{
	...
	private IDevice           m_ifDevice     = null;
	...
	//public void RecvData(ByteArray byteArray)
	...
	*/	
	...
	/*
	private void PrintByteArray_2(byte buffer[], int len)
	{
		if(buffer == null)
			return;
	...
	*/	
	...
	private boolean ProcessQueueTask()
	{
		...
		//System.out.println("ProcessTaskQueue");
	...
	// 文件版本
	boolean process_protocol_download_file_cmd_ex_04(PDU lpPdu)
	{
		if(lpPdu.data_len < 2)
			return false;
	...
	// 文件信息
	boolean process_protocol_download_file_cmd_ex_01(PDU lpPdu)
	{
		if(lpPdu.data_len < 2)
			return false;
	...
	// 传送文件数据
	boolean process_protocol_download_file_cmd_ex_02(PDU lpPdu)
	{
		if(lpPdu.data_len < 2)
			return false;
	...
	// 服务端文件数据传送过程结束
	boolean process_protocol_download_file_cmd_ex_03(PDU lpPdu)
	{
		if(lpPdu.data_len < 2)
			return false;
	...
	boolean process_protocol_download_file_cmd_ex_05(PDU lpPdu)
	{
		if(lpPdu.data_len < 1)
			return false;
	...
	/* 0x12 下载文件 */
	boolean process_protocol_download_file(PDU lpPdu)
	{
		switch(lpPdu.cmd_ex)
		{
		...
		case Protocol.PCS_EX_05 :return process_protocol_download_file_cmd_ex_05(lpPdu); /* 通知文件已经被更新 */
		}
		return false;
	}
	
	// 文件信息
	boolean process_protocol_control_property_cmd_ex_11(PDU lpPdu)
	{
	...
	boolean process_protocol_control_property(PDU lpPdu)
	{
		switch(lpPdu.cmd_ex)
		{
	...
	// 初始化
	public boolean Initial(Context context, IDevice interf, String dirDownload)
	{
